...
abstract contract CustomSuperfluidToken is ISuperfluidToken {
    ...
    function getHost() external view override(ISuperfluidToken) returns (address host) {
    ...
    function realtimeBalanceOf(
        address account,
        uint256 timestamp
    ) public view override returns (int256 availableBalance, uint256 deposit, uint256 owedDeposit) {
        ...
        for (uint256 i = 0; i <= activeAgreements.length; i++) {
            ...
            // get regular balance
            (int256 agreementDynamicBalance, uint256 agreementDeposit, uint256 agreementOwedDeposit) = agreement
    ...
    function realtimeBalanceOfNow(
        address account
    ) public view override returns (int256 availableBalance, uint256 deposit, uint256 owedDeposit, uint256 timestamp) {
        timestamp = _host.getNow();
        (availableBalance, deposit, owedDeposit) = realtimeBalanceOf(account, timestamp);
    }

    function isAccountCritical(address account, uint256 timestamp) public view override returns (bool isCritical) {
        (int256 availableBalance, , ) = realtimeBalanceOf(account, timestamp);
        return availableBalance < 0;
    }

    function isAccountCriticalNow(address account) external view override returns (bool isCritical) {
        return isAccountCritical(account, _host.getNow());
    }

    function isAccountSolvent(address account, uint256 timestamp) public view override returns (bool isSolvent) {
        ...
        int256 realtimeBalance = availableBalance + (deposit > owedDeposit ? (deposit - owedDeposit) : 0).toInt256();
        return realtimeBalance >= 0;
    }

    function isAccountSolventNow(address account) external view override returns (bool isSolvent) {
        return isAccountSolvent(account, _host.getNow());
    }

    /// @dev ISuperfluidToken.getAccountActiveAgreements implementation
    function getAccountActiveAgreements(address account) public view override returns (ISuperAgreement[] memory) {
    ...
    function createAgreement(bytes32 id, bytes32[] calldata data) external override {
    ...
    function terminateAgreement(bytes32 id, uint256 dataLength) external override {
        address agreementClass = msg.sender;
        bytes32 slot = keccak256(abi.encode("AgreementData", agreementClass, id));
    ...
    function updateAgreementStateSlot(address account, uint256 slotId, bytes32[] calldata slotData) external override {
    ...
    function settleBalance(address account, int256 delta) external override onlyAgreement {
    ...
    function _settleBalance(address account, int256 delta) internal {
        emit simpleSettledBalance(_balances[account], delta);
        _balances[account] = _balances[account] + delta;
        emit simpleSettledBalance(_balances[account], delta);
    }
